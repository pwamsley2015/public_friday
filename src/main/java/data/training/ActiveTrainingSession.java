...
public class ActiveTrainingSession {
	...
	public ActiveTrainingSession(TrainingBlock block, int restTime, User user) {
		this.block = block;
	...
	public ActiveTrainingSession(TrainingBlock block, User user) {
		this(block, 5, user);
	}
		...
		switch (state) {
			...
			case WORKING_SETS:
				...
				if (session.currentMovementHasNextSet()) {
					...
					lastSetReminder = new NextSetReminder(next, restTime, user);
	...
	public static class NextSetReminder extends ScheduledFridayAction {
		private static final long serialVersionUID = -3316935746106130162L;
		
		private int restTime;
		
		public NextSetReminder(Date scheduledTime, int restTime, User user) {
			super(scheduledTime, user);
			this.restTime = restTime;
		...
		public FridayOutput<String> execute() {
			return new FridayOutput<>("Sorry to nag you, but it's been " + restTime + " minutes since your last set. You don't want to be here all day, do you? Don't reply to that... just do your next set.");
		}
